...
기대 결과: 초록색 `전체 통과 — 34건`. 실패 0건. 특히 `시군구 + 지명 조합이 반드시 생성됨`이 PASS여야 한다 — 이것이 결함 3.2의 회귀 방지선이다.
...
기대 결과: Task 1·2의 34건은 통과하고, 게이트 5개 그룹이 `FAIL ... (예외)`로 실패한다. 상세에 `Gate is not defined`가 표시된다.
